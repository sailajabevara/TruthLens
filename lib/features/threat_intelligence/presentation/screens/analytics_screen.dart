...
class AnalyticsScreen extends StatelessWidget {
  ...
  (int, int, int, int) _calculateRiskBreakdown(List<ScanRecord> history) {
    ...
    double urlRiskTotal = 0;
    int urlCount = 0;
    double contentRiskTotal = 0;
    double senderRiskTotal = 0;
    double patternRiskTotal = 0;

    for (final record in history) {
      final baseRisk = (100 - record.trustScore).clamp(0, 100);

      // URL/Domain risk based on URL scans only
      if (record.scanType == ScanType.url) {
        urlRiskTotal += baseRisk;
        urlCount++;
      }

      // Content risk increases with suspicious explanations and lower trust scores.
      final explanationPenalty =
          record.explanation.where((line) => line.trim().isNotEmpty).length * 2;
      contentRiskTotal += (baseRisk + explanationPenalty).clamp(0, 100);

      // Sender reputation now derives from real scan outcomes instead of fixed values.
      final senderPenalty =
          record.isLikelyScam ? 25 : (record.riskLevel == RiskLevel.medium ? 10 : 0);
      senderRiskTotal += (baseRisk + senderPenalty).clamp(0, 100);

      final patternBase = switch (record.riskLevel) {
        RiskLevel.high => 85,
        RiskLevel.medium => 55,
        RiskLevel.low => 20,
      };
      patternRiskTotal += ((patternBase + baseRisk) / 2).clamp(0, 100);
    }

    final avgUrlRisk = urlCount == 0 ? 0 : (urlRiskTotal / urlCount).round();

    return (
      avgUrlRisk.clamp(0, 100),
      (contentRiskTotal / history.length).round().clamp(0, 100),
      (senderRiskTotal / history.length).round().clamp(0, 100),
      (patternRiskTotal / history.length).round().clamp(0, 100),
    );
  }
